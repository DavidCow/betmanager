...
public class TableTipsterNameController extends Observable{
	private MainController mainC;
	public static int OPTIONS_TABLETIPSTERNAME_ID = 1124;
	...
	public void init(MainController mainC) {
		this.mainC = mainC;
		inflateTable(TableTitles.TABLE_TITLES_TIPSTERNAME);
	...
	private void inflateTable(String [] tableTitles) {
		System.out.println("Inflating Tipster Name table...");
		...
		for(int i = 0; i<tableTitles.length; i+=1) {
			...
			newTC.setCellValueFactory(new PropertyValueFactory<StatsRow, Object>(TableTitles.tipsterNameTableValueNames[i]));
		 ...
		 */
		System.out.println("Reading Tipster Name Data...");
		List<StatsRow> rows = this.mainC.getStatsCalc().getTipsterStats();
		System.out.println("Reading Tipster Name Data Done!");
	...
	private void notifyMainController() {
		setChanged();
		notifyObservers(new ObservableMessage(OPTIONS_TABLETIPSTERNAME_ID, null)); 
	}
