...
public class TableDayWeekController extends Observable{
	private MainController mainC;
	public static int OPTIONS_TABLEDAYWEEK_ID = 1135;
	...
	public void init(MainController mainC) {
		this.mainC = mainC;
		inflateTable(TableTitles.TABLE_TITLES_DAYWEEK);
	...
	private void inflateTable(String [] tableTitles) {
		System.out.println("Inflating Day/Week table...");
		...
		for(int i = 0; i<tableTitles.length; i+=1) {
			...
			newTC.setCellValueFactory(new PropertyValueFactory<StatsRow, Object>(TableTitles.dayWeekTableValueNames[i]));
		 ...
		 */
		System.out.println("Reading Day/Week Data...");
		List<StatsRow> rows = this.mainC.getStatsCalc().getWeekStats();
		System.out.println("Reading Day/Week Data Done!");
	...
	private void notifyMainController() {
		setChanged();
		notifyObservers(new ObservableMessage(OPTIONS_TABLEDAYWEEK_ID, null)); 
	}
